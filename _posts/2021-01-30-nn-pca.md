...
<!--There are two quite different views that we can have on the brain.--> 
...
> The covariance of two random variables $$x_1$$ and $$x_2$$ is defined as $$\text{cov}(x_1, x_2) = \mathbb{E}[(x_1 - \mathbb{E}[x_1])(x_2 - \mathbb{E}[x_2])]$$, which is equal to $$\mathbb{E}[x_1x_2]$$ if the variables have been centered beforehand so that they have zero mean. The entry at the $$i^{\text{th}}$$ row, $$j^{\text{th}}$$ column of the covariance matrix of a vector of random variables $$\mathbf{x} = (x_1, ..., x_n)$$ is given by $$\text{cov}(x_i, x_j)$$. The expectation over a random variable can be approximated by an average over data points sampled i.d.d. from the distribution of the random variable. Thus, for centered data, $$\text{cov}(x_i, x_j) \approx \frac{1}{m}\sum_{k=1}^m x_i^{(k)}x_j^{(k)}$$ and the covariance matrix can be approximated by $$\frac{1}{m}X^TX$$.
